...
public abstract class FutronicSdkBase
{
    ...
    public static String SdkRetCode2Message(int nRetCode)
    {
        ...
        {
        case RETCODE_OK:
            szMessage = new String( "La función fue completa con éxito." );
            break;

        case RETCODE_NO_MEMORY:
            szMessage = new String( "No hay suficiente memoria para continuar con la ejecución de un programa." );
            break;

        case RETCODE_INVALID_ARG:
            szMessage = new String( "Algunos parámetros no se especificaron o tenían valores no válidos.");
            break;

        case RETCODE_ALREADY_IN_USE:
            szMessage = new String( "La operación actual ya ha inicializado la API." );
            break;

        case RETCODE_INVALID_PURPOSE:
            szMessage = new String( "La plantilla base no corresponde al propósito.");
            break;

        case RETCODE_INTERNAL_ERROR:
            szMessage = new String( "Error interno del sistema de API de Win32 o SDK.");
            break;

        case RETCODE_UNABLE_TO_CAPTURE:
            szMessage = new String( "No se pudo capturar." );
            break;

        case RETCODE_CANCELED_BY_USER:
            szMessage = new String( "Operación cancelada por el usuario." );
            break;

        case RETCODE_NO_MORE_RETRIES:
            szMessage = new String( "El número de reintentos se ha desbordado." );
            break;

        case RETCODE_INCONSISTENT_SAMPLING:
            szMessage = new String( "El muestreo de la fuente es inconsistente." );
            break;

        case RETCODE_FRAME_SOURCE_NOT_SET:
            szMessage = new String( "No se ha establecido la fuente del cuadro." );
            break;

        case RETCODE_DEVICE_NOT_CONNECTED:
            szMessage = new String( "El dispositivo de fuente de fotogramas no está conectado." );
            break;

        case RETCODE_DEVICE_FAILURE:
            szMessage = new String( "Fallo del dispositivo." );
            break;

        case RETCODE_EMPTY_FRAME:
            szMessage = new String( "Marco vacío." );
            break;

        case RETCODE_FAKE_SOURCE:
            szMessage = new String( "Fuente falsa." );
            break;

        case RETCODE_INCOMPATIBLE_HARDWARE:
            szMessage = new String( "Hardware incompatible." );
            break;

        case RETCODE_INCOMPATIBLE_FIRMWARE:
            szMessage = new String( "Firmware incompatible." );
            break;

        case RETCODE_TRIAL_EXPIRED:
            szMessage = new String( "Limitación de la prueba: solo se pueden verificar / identificar 1000 plantillas." );
            break;

        case RETCODE_FRAME_SOURCE_CHANGED:
            szMessage = new String( "Se ha cambiado la fuente de fotogramas." );
            break;

        default:
            szMessage = String.format( "Código de error desconocido %d.", nRetCode );
            break;
        }
